...
// for문이 최대 4개까지 도니깐 시간도 메모리도 많이 쓰는 것 같다
// 다른사람 풀이 봐보자
public class HumanMigration_다른풀이 {
	private static int N, L, R;
	private static int[][] map;
	private static boolean[][] visited;
	private static List<Pair> borderList = new ArrayList<>();
		...
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < N; j++) {
				for(int k = 0; k < 4; k++) {
					...
					if(X >= 0 && X < N && Y >= 0 && Y < N) {
						if(Math.abs(map[i][j]-map[X][Y]) >= L && Math.abs(map[i][j]-map[X][Y]) <= R) {
							return true;
						}
					}
				}
