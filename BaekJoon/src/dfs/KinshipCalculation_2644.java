...
본적으로 부모와 자식 사이를 1촌으로 정의하고 이로부터 사람들 간의 촌수를 계산한다. 
...
public class KinshipCalculation_2644 {
	private static int[][] kinship;
	private static boolean[] visited;
	private static int n;
	private static int count = 0;
	private static boolean find = false;
	
	private static void kinshipRelation(int a, int b, int cnt) {
		if(a == b) {
			find = true;
			count = cnt;
			return;
		...
		for(int i = 1; i <= n; i++) {
			if(kinship[a][i] == 1 && !visited[i]) {
				visited[i] = true;
				if(!find) {
					kinshipRelation(i, b, cnt+kinship[a][i]);					
				}
			}
		}
		return;
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < m; i++) {
			...
			kinship[x][y] = kinship[y][x] = 1;
		}
		
		kinshipRelation(a, b, 0);
