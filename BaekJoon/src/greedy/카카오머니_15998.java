...
public class 카카오머니_15998 {
	...
	private static long IOMoney[][];
	private static long KM = 0;		// 카카오머니
	private static long M = 1;
	private static List<Long> MList = new ArrayList<Long>();
	...
	public static void main(String[] args) throws IOException {
		...
		IOMoney = new long[N][2];
		
		for(int i = 0; i < N; i++) {
			StringTokenizer st = new StringTokenizer(br.readLine());
			IOMoney[i][0] = Long.parseLong(st.nextToken());
			IOMoney[i][1] = Long.parseLong(st.nextToken());
		}
		
		firstWithdraw();
//		printList();
		findDivisor();
		if(MList.isEmpty()) MList.add(1L);
		IOLog();
...
//				System.out.println(i + ", " + KM);
	...
	private static void firstWithdraw() {
		for(int i = 0; i < N; i++) {
			if(IOMoney[i][0] < 0) {
				if(i == 0) {
					M = IOMoney[i][1] - IOMoney[i][0];
					break;
				}else {
					if(IOMoney[i-1][1] + IOMoney[i][0] < 0) {
						M = IOMoney[i][1] - IOMoney[i][0] - IOMoney[i-1][1];		// M의 최대 값
						break;						
					}
				}
			}
