...
2 초   512 MB   15479   5723   4535   36.170%
문제
...
public class 주유소_13305 {
    ...
    private static int[] cityRoad;
    private static int[] oilPrice;
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        N = Integer.parseInt(br.readLine());
        cityRoad = new int[N - 1];
        oilPrice = new int[N];

        StringTokenizer st = new StringTokenizer(br.readLine());
        for (int i = 0; i < N - 1; i++) {
            cityRoad[i] = Integer.parseInt(st.nextToken());
        }
        ...
        System.out.println(getMinMoveCost());

        br.close();
    }

    private static long getMinMoveCost() {
        long min = oilPrice[0];
        long cost = cityRoad[0] * min;
        for (int i = 1; i < N - 1; i++) {
            if (min > oilPrice[i]) {
                min = oilPrice[i];
            }
