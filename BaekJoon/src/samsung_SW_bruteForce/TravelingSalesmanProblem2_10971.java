...
2 초	256 MB	23295	8317	4953	34.697%
문제
...
public class TravelingSalesmanProblem2_10971 {
...
//		Arrays.fill(minCost, MIN_VALUE);
	...
	private static void circulation(int depth) {
		if(depth == N) {
			sum = 0;
			for(int i = 0; i < N; i++) {
				int cost = map[result[i]][result[(i+1)%N]];
				if(cost == 0) {
					sum = minCost;
					break;
				}
