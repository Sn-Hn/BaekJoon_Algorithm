...
public class 소문난칠공주_1941 {
	...
	private static void sevenPricesses(int index, int depth) {
		if(depth == 7) {
			if(isfourCheck() && isAdjacentCheckDFS()) {
				count += 1;			
			}
	...
	private static boolean isAdjacentCheckDFS() {
		v = new boolean[5][5];
	...
	private static void isDFS(int x, int y) {
		for(int i = 0; i < 4; i++) {
			int X = x + dx[i];
			int Y = y + dy[i];
	...
	// 반드시 인접
	// 0 2 ... 시작하면 건너 뛰어 버린다.
	// 7자리를 다 뽑은 후 dfs로 판별해야 할 것 같다
	// 실패 코드
	private static boolean isAdjacentCheck(int depth) {
		Pair pre, now;
		boolean flag = true;
		now = list.get(result[depth]);
		for(int i = 0; i < depth; i++) {
			pre = list.get(result[i]);
			if(Math.abs(pre.x - now.x) + Math.abs(pre.y - now.y) > 1) {
				flag = false;
			}else {
				flag = true;
				break;
